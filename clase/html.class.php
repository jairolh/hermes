...
//                        var_dump($this->cuadro_campos);exit;
		...
		if($this->cuadro_campos>0)
		{
			...
			list ($clave, $val)=each($this->cuadro_registro);
                        ...
                        $this->columnas=(sizeof($val)/2);
                        //se debe dividir las columnas en 2 para no repetir campos 
			//debido a la actualizacion de la clase bdms, mysql y oci de utilizar BOTH para la busqueda
			...
			switch($evento)
			{
				...
				case 3:					
					...
					break;
				case 4 :
					//envio de 2 parametros
					$mi_evento="onchange=\"".$configuracion["ajax_function"]."(document.getElementById('".$configuracion["ajax_control"]."').value,document.getElementById('".$configuracion["ajax_control2"]."').value)\"";
					break;
				default:
					$mi_evento="";			
			...
			if($id=="")
			{
				$this->mi_cuadro="<select name='".$nombre."' size='1' ".$mi_evento." style='font-size:10' tabindex='".$tab."'>\n";
			}
			else
			{
				$this->mi_cuadro="<select id='".$id."' name='".$nombre."' size='1' ".$mi_evento." style='font-size:10' tabindex='".$tab."'>\n";
			}
			...
			{--se reemplaza este for por el foreach*/
                     foreach ($this->cuadro_registro as $key => $value) 
                          {
 				...
 				if($seleccion<0)
				{
					...
					for($this->otro_contador=1;$this->otro_contador<$this->columnas;$this->otro_contador++)
					{
						...
						$this->cuadro_contenido.=$this->cuadro_registro[$key][$this->otro_contador]." ";
						if($limitar==1)
						{
							$this->mi_cuadro.="<option value='".$this->cuadro_registro[$key][0]."'>".substr($this->cuadro_contenido,0,50)."</option>\n";
						}
						else
						{
							$this->mi_cuadro.="<option value='".$this->cuadro_registro[$key][0]."'>".htmlentities($this->cuadro_contenido)."</option>\n";
						}
					}
				...
				else
				{
					for($this->otro_contador=1;$this->otro_contador<$this->columnas;$this->otro_contador++)
					{
						...
						$this->cuadro_contenido.=$this->cuadro_registro[$key][$this->otro_contador]." ";
                                                $valor = $this->cuadro_registro[$key][0];
                                                
						if($limitar==1)
						{ 
							if($this->cuadro_registro[$key][0]==$seleccion)
							{
								$this->mi_cuadro.="<option selected='true' value='".$valor."'>".$this->formato->unhtmlentities(substr($this->cuadro_contenido,0,50))."</option>\n";
							}
							else
							{
								$this->mi_cuadro.="<option value='".$valor."'>".$this->formato->unhtmlentities($this->cuadro_contenido)."</option>\n";
							}
						}
						else
						{
							if($this->cuadro_registro[$key][0]==$seleccion)
							{ 
								$this->mi_cuadro.="<option selected='true' value='".$valor."'>".$this->formato->formatohtml($this->cuadro_contenido)."</option>\n";
							}
							else
							{
								$this->mi_cuadro.="<option value='".$valor."'>".$this->formato->formatohtml($this->cuadro_contenido)."</option>\n";
							}
						}
