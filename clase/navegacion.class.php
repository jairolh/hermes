...
class navegacion
{
				...
				<td >
					...
					<tr class="bloquecentralcuerpo">
						...
						<td align="center" class="celdatabla"><?
							if($hojas>1)
							{
								echo $this->formulario_navegacion($hojas, $variable);
							}
							else
							{
	...
	function formulario_navegacion($hojas, $variable)
	{ 
                ...
                if(is_array($variable))
		{
			foreach ($variable as $clave => $valor) 
			{
				//$mi_variable.=" ".$clave."=".$valor;	
                        if($clave=='bloque')
                            {$formulario=$valor."_hoja";
                             $mi_variable.="<input type='hidden' name='action' value='".$valor."'>\n";
                            }
                        elseif($clave!='bloque' && $clave!='hoja')
                            { $mi_variable.="<input type='hidden' name='".$clave."' value='".$valor."'>\n";
		...
		$cadena_html.="Hoja  <input class='celdanavegacion' type='text' name='hoja' size='2' maxlength='6' value='".$_REQUEST["hoja"]."' onmouseover='return escape(\"".$texto_ayuda."\")' onkeypress=\"if ((event.which == 13) || (event.keyCode == 13) || (event.which == 10) || (event.keyCode == 10)){return(".$verificar.")?document.forms['".$formulario."'].submit():false}\"> de ".$hojas."\n";	
		$cadena_html.="</form>";
