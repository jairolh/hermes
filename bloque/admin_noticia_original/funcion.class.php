...
class admin_noticia implements funcionRegistro
{
	...
	if($id==0)
		...
		{
		$this->cadena_sql=$this->sql->cadena_sql($configuracion,"general",$id);
		$this->count_sql=$this->sql->cadena_sql($configuracion,"contar",$id);
		}
		//echo $this->count_sql;
	  ...
	  //Si no se viene de una hoja anterior
		...
		//Rescatar una hoja específica
				...
				else
				{
					...
					$this->lista_registro_noticia($configuracion,$registro,$campos,$tema,$this->cripto);
					$menu->menu_navegacion($configuracion,$_REQUEST["hoja"],$hojas,$variable);
				}
				
		if(isset($_REQUEST['borrado']))		
			{switch ($_REQUEST['borrado'])
				{case 1: $this->borrarNoticia(1);
					 break;	
				 default: $this->borrarNoticia(0);
					 break;	 	
				}
			}	
	}
	...
	function lista_registro_noticia($configuracion,$registro,$campos,$tema,$cripto)
	{ 
...
function editar_registro_noticia($configuracion,$registro,$tema,$cripto)
			...
			<tr class='bloquecentralcuerpo' onmouseover="setPointer(this, <? echo $contador ?>, 'over', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmouseout="setPointer(this, <? echo $contador ?>, 'out', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');" onmousedown="setPointer(this, <? echo $contador++ ?>, 'click', '<? echo $tema->celda ?>', '<? echo $tema->apuntado ?>', '<? echo $tema->seleccionado ?>');">
				...
				<td bgcolor='<? echo $tema->celda ?>' colspan="3">
					...
					<script type="text/javascript">
						...
						editor_html('noticia',' bold italic underline | left center right |  color hilite rule | number bullet indent outdent | undo redo  | link wikilink image table | word clean html spellcheck');
					</script>
				</td>
			</tr>
	...
	function borrarNoticia($borrado)
	{
	if($borrado==1)
		{
