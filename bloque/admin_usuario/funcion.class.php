...
__________________________________________________________________________________________________*/
	...
	function form_usuario($configuracion,$registro,$tema,$estilo)
	{
		...
		<tr>
			...
			<td>
				<input type='text' name='ext' value='<? echo $registro[0][4] ?>' size='40' maxlength='50' tabindex='<? echo $tab++ ?>' onKeyPress="return solo_numero(event)">
			</td>
		</tr>	
		<tr>
			<td width='30%'><?
...
>
	...
	function editarContrasena($configuracion){
		...
		if(@$usu)
		{
			//VARIABLES PARA EL LOG
			$registro[0] = "EDITAR";
			$registro[1] = $_REQUEST["cod_usuario"];
			$registro[2] = "CONTRASEÑA USUARIO";
			$registro[3] = $_REQUEST["cod_usuario"];
			$registro[4] = time();
			$registro[5] = "Modifica la contraseña del usuario ". $_REQUEST["cod_usuario"];
			$this->log_us->log_usuario($registro,$configuracion);
			unset($_REQUEST['action']);		
