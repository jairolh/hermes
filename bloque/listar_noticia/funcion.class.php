...
class lista_noticia implements funcionRegistro
{
	...
	$this->cadena2_sql=$this->sql->cadena_sql($configuracion,"general","DEVOLUCIONES");
				...
				<?
				  if(!is_array($registro1))
					{	
						...
						$cadena="En la actualidad no hay información publicada.";
						$cadena=htmlentities($cadena, ENT_COMPAT, "UTF-8");
						alerta::sin_registro($configuracion,$cadena);	
					}
					else 
				...
				<?
				  if(!is_array($registro2))
					{	
						...
						$cadena="En la actualidad no hay devoluciones de tramites.";
						$cadena=htmlentities($cadena, ENT_COMPAT, "UTF-8");
						alerta::sin_registro($configuracion,$cadena);	
					}
					else 
...
//@ Método que permite la visualizacion de las noticias publicadas en el portal.
	function lista_registro_noticia($configuracion,$registro,$campos,$tema,$acceso)
	{ 
	...
	<?
			...
			$this->tipo=$this->acceso_db->ejecutarAcceso($this->cadena_sql,"busqueda");
				?>	
					<tr>
						<td class="cuadro_plano">
							<? echo substr($registro[$contador][3],0,200)."..."; ?>
						</td>
					</tr>
					</table>
					</td>
