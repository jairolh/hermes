...
function comparar(formulario)
{
	...
	for(a=0;a<(mi_matriz.length-1);a++)
	{
		...
		if(resultado==1)
		{
			alert1("Se esperaba no tener valores duplicados:"+mi_matriz[a]+'-'+mi_matriz[(a+1)],formulario.elements[control]);
			return false;
		}
...
function verificar_numero(formulario,control)
{
    ...
    if(isNaN(campo.value))
     {
        //formulario.reset();
        campo.select();
        alert1('Debe ingresar un valor numérico',campo);
        campo.focus();
        return false;
    }
    ...
    document.write("<SCR" + "IPT LANGUAGE='JavaScript' SRC='jquery.ui.draggable.js' TYPE='text/javascript'><\/SCR" + "IPT>");
    document.write("<SCR" + "IPT LANGUAGE='JavaScript' SRC='jquery-1.4.2.js' TYPE='text/javascript'><\/SCR" + "IPT>");
    document.write("<SCR" + "IPT LANGUAGE='JavaScript' SRC='jquery-ui.js' TYPE='text/javascript'><\/SCR" + "IPT>");
    document.write("<SCR" + "IPT LANGUAGE='JavaScript' SRC='jquery.alerts.js' TYPE='text/javascript'><\/SCR" + "IPT>");

    function cuadro_alerta1(mensaje,titulo,redireccion,campo){
            jAlert(mensaje,titulo, function(r){
                                            if(redireccion!=''){
...
function alert1(x, campo)
{
    //alert(acentos(x))
    cuadro_alerta1(acentos(x),'Bodega de Datos','',campo)
...
function control_vacio(formulario,control)
{
    ...
    if (isEmpty) {
        ...
        alert1('Todos los campos obligatorios deben ser diligenciados',campo);
        // cuadro_alerta1(acentos('Todos los campos obligatorios deben ser diligenciados'),'Bodega de Datos','',campo)
...
function verificar_rango(formulario,control, min, max)
{
	...
	{	
		...
		{
			campo.select();
			alert1('El valor introducido debe ser un número',campo);
			campo.focus();
			return false;
		}
		else
		{
			...
			{
				campo.select();
				alert1(' Se espera un valor entre ' + min + ' y ' + max,campo);
				campo.focus();
				return false;
			}
			else
...
function verificar_rango_id(formulario,control,mensaje,min, max)
{
	...
	{	
		...
		else
		{
			...
			{
				campo.select();
				alert1(' Se espera un valor entre ' + min + ' y ' + max + 'para el campo:' + mensaje,campo);
				campo.focus();
				return false;
			}
			else
...
function comparar_contenido(formulario,control_1,control_2)
{
    ...
    if (campo_1.value !=campo_2.value) {
        campo_1.value="";
	campo_2.value="";
        alert1('La clave y confirmación no coinciden. Por favor reintente',campo_1);
        campo_1.focus();
	return false;
    } 
...
function longitud_cadena(formulario,control,tamanno)
{
    ...
    if (longitud<tamanno) {
        campo.select();
	alert1('Dato incorrecto. El campo debe contener un dato con mínimo '+tamanno+' caracteres',campo);
        campo.focus();
	return false;
    } 
...
/***********************************************************************************************************
*funcion que muestra el calendario en ventana emergente -ojo va con carpeta calendario ubicada en funcion-*
************************************************************************************************************/
...
function solo_numero(evt){
		...
		if (charCode > 31 && (charCode < 48 || charCode > 57 )&& (charCode < 8 || charCode > 8))
				return false;
		if(evt.ctrlKey) 
				return false;
		return true; 
...
function solo_texto_sin_esp(evt){
		var charCode = (evt.which) ? evt.which : event.keyCode
		if ((charCode < 91 && charCode > 64)  || (charCode < 123 && charCode > 96) ||  (charCode < 166 && charCode > 159) ||  charCode==8  ||  charCode==32 ||  charCode==13 || charCode==241 || charCode==209){
			if(evt.ctrlKey ) {
				return false;
			}else{
				return true;
...
function solo_alfanumerico(evt){
		...
		if ((charCode > 64 && charCode < 91) || (charCode > 96 && charCode < 123) || (charCode > 47 && charCode < 58) || (charCode > 163 &&  charCode < 166) || charCode == 8){
			if(evt.ctrlKey) 
				return false;
			else
				return true;
...
function fecha(formulario,fecha_for,fecha2){
	...
	if (f1>f2) 
			{campo.select();
            alert1('No es posible Guardar con la fecha '+f_inicial+', ya que es mayor a la del día de hoy '+fecha2,campo);
	        campo.focus();
	        return false;
			}
		return true;	
...
function habilitar_boton(formulario,control){

	var campo = formulario.elements[control];
	campo.disabled = false;
        
}//finfuncion habilita_boton

/*******************************************************************************************
*Funcion que no permite el ingreso de tecla control
********************************************************************************************/
...
/*******************************************************************************************
*Funcion que permite validar un numero telefonico para impedir ingreso de 0000000
********************************************************************************************/

function verificar_telefono(formulario,control) {
    	
    	var campo= formulario.elements[control];
   	var num = formulario.elements[control].value;
	
   	if (num <= 1111111){
		campo.select();
		alert1('Número telefónico incorrecto.',campo);
		campo.focus();
		return false;
    	}	 
	return true;
...
/*******************************************************************************************
*Funcion que permite solo el ingreso de a-z y 0-9
********************************************************************************************/
		
function alfanumerico_basico(evt){
...
/*******************************************************************************************
*Funcion que valida los acentos
********************************************************************************************/

function verificar_acentos(formulario,control) {
	var campo = formulario.elements[control];
	var cadena = campo.value;
	var acentos = "`´'\"^áéíóúàèìòùäëïöüâêîôûñÁÉÍÓÚÀÈÌÒÙÂÊÎÔÛÄËÏÖÜÑ";
	
   for(i=0; i<cadena.length; i++){
      if (acentos.indexOf(cadena.charAt(i),0)!=-1){
		campo.select();
		alert('Contiene caracteres no permitidos.',campo);
		campo.focus();
         	return false;
      }
   }
...
/*******************************************************************************************
*Funcion que valida los caracteres especiales
********************************************************************************************/

function verificar_caracteres_especiales(formulario,control) {
	var campo = formulario.elements[control];
	var cadena = campo.value;
	var acentos = "!·$%&/()=?¿¡'^*+ºª|@#~¬{[]}\\~`´<>:;,";
...
/*******************************************************************************************
*Funcion que permite verificar un minimo de checkbox seleccionados para el perfil personalizado
********************************************************************************************/

function verificar_Checks(formulario, cantidad_minima, control) {
    if (control){
        var campo = formulario.elements[control];
        var cadena = campo.options[formulario.elements[control].selectedIndex].value;
    }
    resu = 0;
    if (cadena==1){ //checks dependientes de select personalizado
        for (var i = 0, total = formulario.elements.length; i < total; i ++)
            if (formulario.elements[i].type == "checkbox"){
                if  (formulario.elements[i].checked && formulario.elements[i].disabled==false)
                    resu ++;
            }
        if (resu < cantidad_minima){
 		alert1('Seleccione mínimo '+ cantidad_minima +' opcion(es). ',campo);
		//campo.focus();
         	return false;
    ...
    if (!control){//check que no dependen de un valor de select
    		    for (var j = 0, total2 = formulario.elements.length; j < total2; j ++)
            if (formulario.elements[j].type == "checkbox"){
